...
class RunHistoryPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # V9.2：init 也从主题模板取色（原硬编码 #FFFFFF/#FAFBFC/#6B7280）
        _c = COLORS
        ...
        try:
            ...
            if isinstance(widths, list):
                for i, w in enumerate(widths):
                    if i < self.table.columnCount() - 1:  # 最后一列是 Stretch，跳过
                        self.table.setColumnWidth(i, int(w))
        except (ValueError, TypeError):
            logger.warning("恢复运行历史列宽失败，使用默认值")

    def _save_column_widths(self):
        """任务8：保存列宽到 QSettings"""
        widths = [
            self.table.columnWidth(i)
            for i in range(self.table.columnCount() - 1)  # 最后一列是 Stretch，跳过
        ]
    ...
    def _reset_column_widths(self):
        """任务8：重置为默认列宽"""
        for i, (_, width) in enumerate(self.COLUMNS[:-1]):  # 最后一列是 Stretch，跳过
            self.table.setColumnWidth(i, width)
    ...
    def refresh_theme(self, dark: bool):
        ...
        # V9.2：从主题模板取（原硬编码 #2C2C2E/#FAFBFC/#AAAAAA/#6B7280）
        header_bg = colors['surface_header']
        header_color = colors['text_secondary']
        self.cmb_status.setStyleSheet(f"""
            QComboBox {{
                border: 1px solid {colors['border']}; border-radius: 6px;
                padding: 3px 6px; background: {colors['background']};
