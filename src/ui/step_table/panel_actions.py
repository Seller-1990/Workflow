...
def show_context_menu(panel, pos):
    ...
    # 任务9：多选时提供批量操作
    selected_rows = panel._get_selected_step_rows()
    if len(selected_rows) > 1:
        _build_batch_menu(panel, menu, selected_rows, can_edit)
...
def _build_batch_menu(panel, menu, selected_rows, can_edit):
    """任务9：批量操作菜单——批量删除、批量改阶段"""
    action_batch_delete = menu.addAction(f"批量删除（{len(selected_rows)} 项）")
    action_batch_delete.setEnabled(can_edit)
    action_batch_delete.setToolTip("" if can_edit else '需要先开启"编辑"。')
    action_batch_delete.triggered.connect(lambda: panel._batch_delete_steps(selected_rows))

    move_menu = menu.addMenu("批量改阶段")
    move_menu.setStyleSheet(get_menu_stylesheet(panel._dark))
    if not panel._stages or not can_edit:
        move_menu.setEnabled(False)
    else:
        for st in panel._stages:
            act = move_menu.addAction(st["name"])
            act.triggered.connect(lambda _, su=st["uid"]: panel._batch_change_stage(selected_rows, su))
