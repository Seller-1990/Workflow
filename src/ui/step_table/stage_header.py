...
def _create_header_widget(
    ...
    header_widget.setStyleSheet(_header_stylesheet(state))

    layout = QHBoxLayout(header_widget)
    layout.setContentsMargins(10, 4, 10, 4)
    layout.setSpacing(8)
    layout.setAlignment(Qt.AlignVCenter)
    layout.addWidget(_create_accent(state.colors))
...
def _create_accent(colors: dict) -> QFrame:
    accent = QFrame()
...
def _create_title_label(state: StageHeaderState) -> QLabel:
    ...
    title.setStyleSheet(f"color:{state.colors['text_primary']}; font-size:13px; font-weight:600;")
...
def _header_stylesheet(state: StageHeaderState) -> str:
    colors = state.colors
    return f"""
        QWidget#stageHeader {{
            background: {colors['selected_bg'] if state.selected else 'transparent'};
            border: 1px solid {colors['primary'] if state.selected else 'transparent'};
