...
class WorkbenchBoardPanel(QWidget):
    ...
    def _tokens(self) -> dict:
        # V9：全部从全局 COLORS 派生，消除双调色板（原 blue:#2458d3 / panel_soft:#f1eee7）
        # V9.2：purple → violet 统一命名（与 theme.py token 名一致）
        c = get_colors(self._dark)
        return {
            "bg": c["background"],
            "panel": c["surface_card"],
            "panel_soft": c["surface_secondary"],
            "lane": "rgba(255, 255, 255, 0.72)" if not self._dark else "rgba(44, 44, 46, 0.72)",
            "ink": c["text_primary"],
